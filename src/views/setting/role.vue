<template>
  <div class="app-container">
    <div class="filter-container">
      <el-input v-model="listQuery.search.name" placeholder="角色名称" style="width: 200px;" class="filter-item" @keyup.enter.native="handleFilter" />
      <el-select v-model="listQuery.search.permission_id" placeholder="请选择权限" clearable style="width: 120px" class="filter-item">
    ...
    <el-dialog :title="textMap[dialogStatus]" :visible.sync="dialogFormVisible">
      <el-form ref="dataForm" :rules="rules" :model="temp" label-position="left" label-width="120px" style="width: 400px; margin-left:30px;">
        ...
        <el-form-item label="权限" prop="permission_ids">
          <el-select v-model="temp.permission_ids" multiple placeholder="请选择" style="width:400px">
            <el-option
...
<script>
import { getAll, create, update, deleteOne } from '@/api/role'
import { getOption } from '@/api/permission'
  ...
  data() {
    return {
      ...
      rules: {
        ...
        permission_ids: [{ type: 'array', required: true, message: '权限必填', trigger: 'change' }]
      }
    }
  },
  created() {
      ...
      this.$refs['dataForm'].validate((valid) => {
        if (valid) {
          const tempData = {
            id: this.temp.id,
            name: this.temp.name,
            permission_ids: this.temp.permission_ids
          }
